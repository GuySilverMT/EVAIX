...
export const openwebuiRouter = createTRPCRouter({
  ...
  getConfig: publicProcedure.query(async () => {
    return {
      baseUrl: process.env.OPENWEBUI_API_URL || 'http://localhost:3000/api',
      liteLlmUrl: process.env.LITELLM_ARBITRAGE_URL || 'http://localhost:8080/v1',
  ...
  sendMessage: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const liteLlmUrl = process.env.LITELLM_ARBITRAGE_URL || 'http://localhost:8080/v1';
