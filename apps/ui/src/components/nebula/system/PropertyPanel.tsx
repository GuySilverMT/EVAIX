...
export const PropertyPanel = ({ tree: propTree, setTree: propSetTree }: PropertyPanelProps) => {
  const { selectedNodeId, setSelectedNodeId, currentTree, setCurrentTree } = useBuilderStore();
  
  const tree = propTree || currentTree;
  const setTree = propSetTree || setCurrentTree;

  if (!tree || !selectedNodeId || !tree.nodes[selectedNodeId]) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center bg-zinc-950 text-zinc-600 h-full w-full">
        <Sliders size={24} className="mb-2 text-zinc-700 animate-pulse" />
        <span className="text-[10px] uppercase tracking-widest font-bold">Property Manager</span>
        <span className="text-[9px] text-zinc-500 mt-1">Select a component node to edit properties</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full w-full bg-zinc-950 border border-zinc-800 rounded-sm overflow-hidden font-sans">
       <div className="p-3 border-b border-zinc-800 flex justify-between items-center bg-zinc-900">
          <div>
            ...
            <div className="text-[10px] font-mono text-zinc-400 truncate w-36">{node.id}</div>
          </div>
          <button onClick={handleDelete} title="Delete Node" className="text-zinc-500 hover:text-red-400 p-1.5 transition-colors">
             <Trash2 size={12} />
          </button>
       </div>
       ...
       <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
          ...
          <div className="space-y-4">
              {def && Object.entries(def.propSchema).map(([key, schema]: [string, PropSchema]) => (
                <div key={key} className="space-y-1.5">
                    <label className="text-[9px] font-bold text-zinc-400 uppercase tracking-tight ml-1">{key}</label>
          ...
          <div className="pt-6 border-t border-zinc-800 mt-6">
             <div className="text-[9px] font-bold text-zinc-500 uppercase mb-3 ml-1">Properties JSON</div>
             <textarea 
               className="w-full h-48 bg-zinc-900 border border-zinc-800 rounded p-3 text-[10px] font-mono text-zinc-400 focus:text-zinc-200 focus:border-zinc-700 outline-none transition-all resize-none"
