...
import { getRegisteredComponent } from '../../registry/ComponentRegistry.js';

/**
 * @file AppCard.tsx
 * @description The "Agentic Wrapper" for any content node. 
 * Orchestrates WebNode or NativeNode components dynamically based on registry.
 */
...
export const AppCard = memo(({ id, isFocused = true, isCondensed = false }: AppCardProps) => {
  ...
  const viewMode = (card.appId || card.activeTool || card.metadata?.viewMode || 'webnode') as string;
  
  if (isCondensed) {
    return <StackStrip title={viewMode.toUpperCase()} />;
  }

  const RegisteredComponent = getRegisteredComponent(viewMode);
  ...
  return (
    <div className="flex h-full w-full rounded-none border-0 bg-[var(--colors-background)] overflow-hidden relative flex-col">
      ...
      <div className="flex-1 flex overflow-hidden relative w-full h-full min-h-0 min-w-0">
        {RegisteredComponent ? (
           <RegisteredComponent cardId={id} id={id} {...card.props} />
        ) : (
          <div className="h-full w-full flex flex-col items-center justify-center p-6 text-center">
            <span className="text-xs font-semibold text-zinc-400 mb-1">Unregistered Node</span>
            <span className="text-[10px] text-zinc-500 font-mono">"{viewMode}" is not registered in ComponentRegistry.</span>
          </div>
        )}
      </div>
    </div>
