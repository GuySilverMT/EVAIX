...
import { Folder, Database, Server, Plus, ChevronRight, FileText, Activity, Layers } from 'lucide-react';
import { toast } from 'sonner';

/**
 * @file ProjectNavigator.tsx
 * @description The hierarchical directory tree mapping that governs context routing and project storage in EVAIX.
 * Data-driven tree structure supporting infinite project types, starting with 0 active projects on fresh install.
 */
...
export function ProjectNavigator() {
  ...
  const { projects, addProject, projectTypes } = useProjectStore();
  ...
  // RAG-linked Markdown project files
  ...
  const handleCreateProject = (e: React.FormEvent) => {
    ...
    const newProject: ProjectNode = {
      ...
      type: newProjectType,
      path: `/home/user/projects/${newProjectName.toLowerCase().replace(/\s+/g, '-')}`
    };
    addProject(newProject);
    toast.success(`Initialized Project: ${newProject.name} (${newProject.type})`);
    setNewProjectName('');
    ...
    <div className="flex flex-col h-full w-full bg-zinc-950 text-zinc-300 font-sans border-r border-zinc-800">
      ...
      <div className="h-12 border-b border-zinc-800 flex items-center px-4 justify-between bg-zinc-900/50">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Project Navigator</span>
        </div>
        <button 
          onClick={() => setShowNewProjectModal(true)}
          className="text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-1 text-[10px] font-mono text-indigo-400"
          title="Initialize Project Tree"
        >
      ...
      {showNewProjectModal && (
        <form onSubmit={handleCreateProject} className="p-3 border-b border-zinc-800 bg-zinc-900/60 flex flex-col gap-2 animate-in slide-in-from-top duration-150">
          <span className="text-[9px] font-mono font-bold uppercase tracking-widest text-indigo-400">Initialize Project Tree</span>
          <input 
            type="text"
            placeholder="Project Name..."
            value={newProjectName}
            onChange={(e) => setNewProjectName(e.target.value)}
            className="w-full bg-zinc-950 border border-zinc-800 px-2 py-1 text-[11px] font-mono text-zinc-200 focus:outline-none focus:border-indigo-500"
            required
            autoFocus
          />
          <div className="flex gap-2">
            <select 
              ...
              className="flex-1 bg-zinc-950 border border-zinc-800 px-2 py-1 text-[10px] font-mono text-zinc-300 focus:outline-none"
            >
              {projectTypes.map(t => (
            ...
            <button 
              type="submit"
              className="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded-sm"
            >
              Create
            </button>
            <button 
              ...
              className="text-zinc-500 hover:text-zinc-300 px-2 text-[10px]"
            >
              Cancel
            </button>
          </div>
      ...
      {/* Dynamic Project Tree List */}
      <div className="flex-1 overflow-y-auto min-h-[140px]">
        {projects.length === 0 && recentProjects.length === 0 ? (
          <div className="p-6 flex flex-col items-center justify-center text-center text-[11px] text-zinc-600 font-mono gap-2">
            <Layers size={24} className="text-zinc-700" />
            <span>0 Projects Configured</span>
            <button
              onClick={() => setShowNewProjectModal(true)}
              className="mt-1 px-2.5 py-1 bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 text-indigo-400 text-[10px] font-mono rounded"
            >
          ...
          <>
            {projects.map(proj => {
              ...
              return (
                <div 
                  key={proj.id}
                  onClick={() => setActiveWorkspaceId(proj.id)}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[8px] font-mono uppercase text-zinc-600">{proj.type}</span>
                    </div>
                  </div>
...
export const ProjectManagerPanel = ProjectNavigator;
