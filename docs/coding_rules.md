...
- **Naming Convention:**
  ...
  - **Reasoning:** An LLM is a _driver_. The Code is the _vehicle_. Do not confuse the driver with the car.
...
- **Philosophy:** The codebase must remain buildable even if flat JSON files are out of sync or missing fields.
- **Practice:** If the IDE/Build fails on newly added or modified JSON fields, use local **Delegates** or **Interfaces** and `any` casts in the service layer until the environment stabilizes. Do not stop development for a data synchronization lag.

## 11. Tech Stack Constraints

- **Frontend:** Next.js + React (Nebula/Workbench).
- **Backend:** Node.js/TypeScript.
- **Database:** Flat JSON files for application state, PostgreSQL for vector embeddings.
